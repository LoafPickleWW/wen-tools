  ...
  async function getAssetData(asset) {
    try {
      ...
      return {
        ...
        unit_name: response.data.params["unit-name"] || "-",
        asset_name: response.data.params.name || "-",
        amount: (asset.amount / 10 ** response.data.params.decimals).toFixed(
          response.data.params.decimals
        ),
      };
    } catch (err) {
      return "";
  ...
  async function downloadWalletDataAsCSV() {
    if (walletData.length > 0) {
      ...
      exportCSVFile(
        Object.keys(data[0]),
        data,
        `${userWallet}-holding-${networkType}.csv`
      );
      setLoading(false);
      setCounter(0);
